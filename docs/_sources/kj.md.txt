...
实质重于形式要求企业应当按照交易或者事项的经济实质进行会计确认、计量和报告，不仅仅以交易或者事项的法律形式为依据。【举例】企业以融资租赁方式租入固定资产
...
1. 负债的定义
   ...
   （1）负债是企业承担的现时义务；现时义务：是指企业在现行条件下已承担的义务。

   （2）负债的清偿预期会导致经济利益流出企业；（现金、实物资产、提供劳务、转为资本等）

   （3）负债是由企业过去的交易或者事项形成的。

2. 负债的确认条件：将一项现时义务确认为负债，需要符合负债的定义，并同时满足以下两个条件：
...
原则是指按照企业会计准则规定的、适合于企业会计核算所采用的**具体会计原则**

基础是指为了将会计原则应用于交易或者事项而采用的基础，主要是**计量基础**（即**计量属性**），包括历史成本、重置成本、可变现净值、现值和公允价值等

会计处理方法是指企业在会计核算中按照法律、行政法规或者国家统一的会计制度等**规定采用**或者**选择的**、适合于本企业的**具体会计处理方法**

 （二）会计政策具有**以下特点**：

1. 会计政策的**选择性**（存货**发出时**的计价方法）

2. 会计政策应当在会计准则**规定的范围内**选择
...
会计估计：是指企业对结果不确定的交易或者事项以最近可利用的信息为基础**所作的判断**。
...
4. 在**当期期初**确定会计政策变更对以前各期累积影响数**不切实可行**的，应当采用**未来适用法**处理。
...
2. 会计估计变更既影响**变更当期**又影响**未来期间**的，其影响数应当在**变更当期**和**未来期间**予以确认。（**固定资产**的**折旧年限**）。会计估计变更的影响数应计入**变更当期与前期相同的项目中**。
...
前期差错通常包括**计算错误**、**应用会计政策错误**、**疏忽或曲解事实**以及**舞弊**产生的影响。
...
5. 递延所得税
   ...
   - 如果**重要的**前期差错更正**涉及暂时性差异**，**应分别**应纳税暂时性差异与可抵扣暂时性差异，**确认相关的**递延所得税负债与递延所得税资产，并在此基础上**确定**利润表中的**所得税费用**。
...
1. 商品流通企业在采购商品过程中发生的运输费、装卸费、保险费以及其他可归属于存货采购成本的费用等：（3种做法）
   - 应当计入存货采购成本。
...
1. 原理：移动加权平均法：是指在每次收货以后，立即根据库存存货数量和总成本，计算出新的平均单位成本的一种计算方法。计算公式：
...
1. 资产负债表日，存货应当按照成本与可变现净值孰低计量。其中：
...
1. 为执行销售合同或者劳务合同而持有的存货，通常应以产成品或商品的合同价格作为其可变现净值的计算基础。
   ...
   ```text
   - 如果企业就其产成品或商品签订了销售合同，则该批产成品或商品的可变现净值应以合同价格作为   计算基础。总体原则：有合同的按合同价，没有合同的按市场价。
   - 如果企业销售合同所规定的标的物还没有生产出来，但持有专门用于该标的物生产的材料，其可变   现净值也应以其生产的产成品的合同价格作为计算基础。
   ```
...
2. 属于自然灾害等非常原因造成的存货毁损，应先扣除处置收入（如残料价值）、可以收回的保险赔偿和过失人赔偿，将净损失计入营业外支出。因非正常原因导致的存货盘亏或毁损，按规定不能抵扣的增值税进项税额，应当予以转出。【特别提示】自然灾害造成外购存货的毁损，其进项税额可以抵扣，不需要转出。
...
1. 出包方式建造固定资产（与建造承包商签订建造合同）（通过“在建工程”核算）

入账价值=建筑工程支出+安装工程支出+安装设备支出+分摊的待摊支出

1. 其他方式取得的固定资产的成本

   （1）投资者投入的固定资产的成本。投资者投入的固定资产的成本，应当按照投资合同或协议约定的价值确定，但合同或协议约定价值不公允的除外。
...
1. 研究阶段（计划性、探索性、较大的不确定性）：研究阶段的有关支出在发生时，应当予以费用化计入当期损益（管理费用）。
2. 开发阶段（符合条件资本化，不符合条件费用化）
3. 无法区分研究阶段和开发阶段的支出：无法区分研究阶段和开发阶段的支出，应当在发生时费用化，计入当期损益。（管理费用）
...
1. 资本化支出
   ...
   达到预定用途时结转。资表日是可以有余额的，填列在   “开发支出”内。
   借：无形资产
   贷：研发支出——资本化支出
...
对于企业外购的房地产，只有在**购入房地产的同时开始对外出租**（自租赁期开始日起，下同）或用于资本增值，才能称之为外购的投资性房地产。外购投资性房地产的实际成本，包括购买价款、相关税费和可直接归属于该资产的其他支出。
...
已采用公允价值模式计量的投资性房地产，**不得**从公允价值模式转为成本模式。
...
- 非转投
  ...
  减值部分（借方差额）：借记-公允价值变动损益

- 投转非

  固定资产或存货按公允价值计量

  公允与账面差：计入公允价值变动损益（可能借可能货）

#### **二、投资性房地产的处置**

当投资性房地产被处置，或者永久退出使用且预计不能从其处置中取得经济利益时，应当终止确认该项投资性房地产。

企业出售、转让、报废投资性房地产或者发生投资性房地产毁损时，应当将处置收入扣除其账面价值和相关税费后的金额计入当期损益（将实际收到的处置收入计入**其他业务收入**，所处置投资性房地产的账面价值计入其他业务成本）。
...
资产减值，是指资产的可收回金额低于其账面价值。本章所指资产，除特别说明外，包括单项资产和资产组。本章涉及的资产减值对象主要包括以下资产：
...
3. 无论是否有确凿证据表明资产存在减值迹象，均应至少于每年年末进行减值测试的有：
...
2. 以记账本位币表示的资产未来现金流量的现值=以外币（结算货币）表示的未来现金流量现值×计算资产未来现金流量现值当日的即期汇率；
3. 以记账本位币表示的资产未来现金流量的现值与资产公允价值减去处置费用后的净额相比较，较高者为其可收回金额，根据可收回金额与资产账面价值相比较，确定是否需要确认减值损失以及确认多少减值损失。
...
| 开发支出                             | 可收回金额                         | 不可以           |
| 商誉                                 | 可收回金额                         | 不可以           |
| 金融工具（FVPV/FVOCI不计提减值准备） | 以预期信用损失为基础，确认损失准备 | 可以             |
...
2. 资产组的认定，应当考虑企业管理层对生产经营活动的管理或者监控方式（如是按照生产线、业务种类还是按照地区或者区域等）和对资产的持续使用或者处置的决策方式等。比如企业各生产线都是独立生产、管理和监控的，那么各生产线很可能应当认定为单独的资产组；如果某些机器设备是相互关联、互相依存的，其使用和处置是一体化决策的，那么，这些机器设备很可能应当认定为一个资产组。
...
1. 计算合并报表中商誉的账面价值：交易价格-交易时可辨认净资产*持股比例
2. 计算完全商誉账面价值：合并报表中的商誉/持股比例
3. 计算资产组包含完全商誉的账面价值：资产组可辨认净资产账面价值加上完全商誉
4. 计算减值额：资产组包含完全商誉的账面价值-可回收金额
5. 计算合并报表商誉减值额：按持股比例首先抵减商誉
...
8. 增值税税控系统专用设备和技术维护费用抵减增值税额的会计处理按增值税有关规定，初次购买增值税税控系统专用设备支付的费用以及缴纳的技术维护费允许在增值税应纳税额中全额抵减。企业购入增值税税系统专用设备，按实际支付或应付的金额，借记“固定资产”科目，贷记“银行存款”、“应付账款”等科目。按规定抵减的增值税应纳税额，借记“应交税费——应交增值税（减免税款）”科目（小规模纳税人借记“应交税费——应交增值税”科目），贷记“管理费用”科目。
...
3. 企业委托加工的应税消费品，委托方收回后以不高于受托方的计税价格出售的，为直接出售，不再缴纳消费税；委托方收回后以高于受托方的计税价格出售的，不属于直接出售，需按照规定申报缴纳消费税，在计税时准予扣除受托方已代收代缴的消费税；委托加工的应税消费品收回后用于连续生产应税消费品，按规定准予抵扣的，记入“应交税费——应交消费税”科目的借方。
   ...
   （2）支付的用于连续生产应税消费品按规定准予抵扣的消费税，不计入存货成本，应记入“应交税费——应交消费税”科目的借方，用以抵减最终完工产品销售时缴纳的消费税。
...
7. 【关注】中国证监会会计部，上市公司执行企业会计准则监管问题解答：上市公司按照税法规定需补缴以前年度税款或由上市公司主要股东或实际控制人无偿代为承担或缴纳税款及相应罚金、滞纳金的，应该如何进行会计处理？
...
1. 发行债券
   ...
   “应付债券——利息调整”科目的发生额也可能在借方。发行债券的发行费用应计入发行债券的初始成   本，反映在“应付债券——利息调整”明细科目中。
...
企业采用融资租赁方式租入的固定资产，租赁期开始日，应按最低租赁付款额，确认长期应付款的入账价值。
...
符合资本化条件的资产在购建或者生产过程中发生**非正常中断**、且中断时间**连续超过3个月**的，应当暂停借款费用的资本化。在中断期间所发生的借款费用，应当计入当期损益，直至购建或者生产活动重新开始。但是，如果中断是使所购建或者生产的符合资本化条件的资产达到预定可使用或者可销售状态必要的程序，所发生的借款费用应当继续资本化。
...
1. 为购建或者生产符合资本化条件的资产而借入**专门借款**的，应当以专门借款**当期实际发生的利息费用**，**减去**将尚未动用的借款资金存入银行取得的**利息收入**或进行暂时性投资取得的**投资收益**后的金额确定。
...
| 利息           | 专门借款                                                     | 一般借款                                                     |
| -------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 资本化利息费用 | 资本化利息费用金额=资本化期间专门借款本金发生的利息金额-资本化期间闲置资金的投资收益或利息收入   （与专门借款的具体支出数无关，资本化期间发生的全部利息费用均资本化） | 资本化利息费用金额=累计资产支出超过专门借款部分的资产支出加权平均数×一般借款资本化率 |
| 费用化利息费用 | 费用化利息费用金额=费用化期间专门借款本金发生的利息金额-费用化期间闲置资金的投资收益或利息收入 | 费用化利息费用金额=一般借款利息总额－一般借款资本化利息费用－一般借款闲置资金的投资收益或利息收入   （闲置资金收益全部冲减费用化利息费用金额） |

在资本化期间内，外币专门借款本金及利息的汇兑差额，应当予以资本化，计入符合资本化条件的资产的成本。外币一般借款的借款费用计入当期损益，不予资本化。
...
1. 累积带薪缺勤
   ...
   企业应当在职工提供服务从而增加了其**未来享有的带薪缺勤权利时**，确认与累积带薪缺勤相关的职工薪酬，并以**累积未行使权利**而增加的**预期支付金额**计量。
...
1. 如果出售住房的合同或协议中未规定职工在购得住房后必须服务的年限，企业应当将该项差额直接计入出售住房当期相关资产成本或当期损益。
...
设定受益计划是指除设定提存计划以外的离职后福利计划。当企业通过以下方式负有法定义务时，该计划就是一项设定受益计划：
...
1. 步骤一：确定设定受益义务现值和当期服务成本。

   企业应当通过下列两步确定设定受益义务现值和当期服务成本。

   （1）根据**预期累计福利单位法**，釆用无偏且相互一致的精算假设对有关人口统计变量（如职工离职率和死亡率）和财务变量（如未来薪金和医疗费用的增加）等作出估计，计量设定受益计划所产生的义务，并确定相关义务的归属期间。
...
3. **步骤三**：**确定应当计入当期损益的金额**。
   ...
   1. **当期服务成本**：
      ...
      是指因职工当期服务导致的设定受益义务现值的增加额。**即为归属于当年福利的现值**。

   2. **过去服务成本**：

      是指设定**受益计划修改**所导致的与以前期间职工服务相关的设定受益计划义务现值的增加或减少。
...
4. **步骤四：确定应当计入其他综合收益的金额。**

   设定受益净负债或净资产的**重新计量**应当计入其他综合收益，且在后续期间不应重分类计入损益，但是企业可以在权益范围内转移这些在其他综合收益中确认的金额。
...
1. 辞退福利，是指企业在职工劳动合同到期之前解除与职工的劳动关系，或者为鼓励职工自愿接受裁减而给予职工的补偿。企业向职工提供辞退福利的，应当在以下两者孰早日确认辞退福利产生的职工薪酬负债，并计入当期损益：

   （1）企业不能单方面撤回解除劳动关系计划或裁减建议所提供的辞退福利时。

   （2）企业确认涉及支付辞退福利的重组相关的成本或费用时。

2. 同时存在下列情况时，表明企业承担了重组义务：

   （1）有详细、正式的重组计划，包括重组涉及的业务、主要地点、需要补偿的员工人数及其岗位性质、预计重组支出、计划实施时间等；

   （2）该重组计划已对外公告。

3. 核算：由于被辞退的职工不再为企业带来未来经济利益，因此，对于所有辞退福利，均应当于辞退计划满足负债确认条件的当期一次计入费用，不计入资产成本。
...
### **第一节　股份支付概述**
 ...
 股份支付，是指企业为获取职工和其他方提供服务而授予权益工具或者承担**以权益工具为基础**确定的负债的交易。
...
1. 换取职工服务的股份支付的确认和计量原则
   ...
   **应按授予日权益工具的公允价值计量，不确认其后续公允价值变动。**
   ...
   对于换取职工服务的股份支付，企业应当以股份支付所授予的权益工具的公允价值计量。企业应在等待期内的每个资产负债表日，以对可行权权益工具数量的最佳估计数为基础，按照权益工具在授予日的公允价值，将**当期取得的服务计入相关资产成本或当期费用，同时计入资本公积中的其他资本公积。**
...
3. 取消或结算
   ...
   将取消或结算作为**加速可行权**处理，立即确认原本应在剩余等待期内确认的金额。
...
2. 分配现金股利的会计处理
   ...
   【关键点】预计未来可解锁，现金股利通过“利润分配”科目核算；现金股利可撤销，应减少其他应付款。后续信息表明不可解锁限制性股票的数量与以前估计不同的，应当作为会计估计变更处理，直到解锁日预计不可解锁限制性股票的数量与实际未解锁限制性股票的数量一致

```text
   现金股利可撤销
   预计未来可解锁：现金股利应当作利润分配处理
   借：利润分配——应付现金股利或利润
   贷：应付股利——限制性股票股利
   同时，按分配的现金股利金额
...
（二）接受服务企业没有结算义务或授予本企业职工的是其本身权益工具的应当将该股份支付交易作为权益结算的股份支付处理；接受服务企业具有结算义务且授予本企业职工的是企业集团内其他企业权益工具的，应当将该股份支付交易作为现金结算的股份支付处理。
...
2. 结算企业（母公司）不是以其本身权益工具结算，接受服务企业（子公司）没有结算义务
   ...
   【提示】合并财务报表中相当于集团会计主体授予集团会计主体职工现金结算股份支付的结果，合并   财务报表中最终体现的是按现金结算股份支付计量原则确认的应付职工薪酬和管理费用等。因接受服   务企业确认的管理费用等是按权益结算股份支付计量原则确定的，所以合并财务报表抵销分录中会出   现差额，该差额计入管理费用等。
...
1. **亏损合同存在标的资产**的，应当对标的资产进行减值测试并按规定确认减值损失，如果预计亏损超过该减值损失，应将超过部分确认为预计负债；

2. **合同不存在标的资产**的，亏损合同相关义务满足预计负债确认条件时，应当确认为预计负债。
...
对于归类为**权益工具**的金融工具，无论其名称中是否包含“债”，**其利息支出或股利分配都应当作为发行企业的利润分配**，其回购、注销等作为权益的变动处理；对于归类为**金融负债**的金融工具，无论其名称中是否包含“股”，其**利息支出或股利分配原则上按照借款费用**进行处理，其回购或赎回产生的利得或损失等计入当期损益。
...
5. 权益工具与金融负债重分类
   ...
   借：其他权益工具——优先股、永续债等（账面价值）
        ...
        　　　　　　　——优先股、永续债等（利息调整）
           　　 （应付债券公允价值与面值的差额）（或借方）
             　　　资本公积——资本溢价（或股本溢价）（重分类日应付债券公允价值与权益工具账   面价值的差额）（或借方）
...
3. 采用权益法核算的长期股权投资
   ...
   ```text
   - 被投资单位其他综合收益增加，投资方按持股比例计算应享有的份额：
   借：长期股权投资——其他综合收益
           贷：其他综合收益
...
4. 存货或自用房地产转换为投资性房地产
   ...
   ```text
      - 企业将作为存货的房地产转为采用公允价值模式计量的投资性房地产，转换日其公允价值大于   账面价值
   借：投资性房地产——成本（转换日的公允价值）
        　　存货跌价准备（如涉及）
        贷：开发产品等
        　　　其他综合收益（差额）

      - 企业将自用房地产转为采用公允价值模式计量的投资性房地产，转换日其公允价值大于账面价   值
   借：投资性房地产——成本（转换日的公允价值）
        　　累计折旧
        　　固定资产减值准备
        贷：固定资产
...
5. 现金流量套期工具产生的利得或损失中属于**有效套期的部分**，直接确认为**其他综合收益**。
...
- 如果企业先收资金再购建，则应在开对相关资产计提折旧或摊销时开始将递收益分期计入损益；
- 如果企业先购建再收到资金，则应在关资产剩余使用寿命内按合理、系统的法将递延收益分期计入损益。
- 需要说明的是，采用总额法的，如果应的长期资产在持有期间发生减值损失递延收益的摊销仍保持不变，不受减值素的影响。企业对与资产相关的政府补选择总额法核算的，应当将递延收益分转入其他收益或营业外收入，借记“递延益”科目，贷记“其他收益”或“营业外入”科目。相关资产在使用寿命结束时或束前被处置（出售、报废、转让、发生损等），尚未分配的相关递延收益余额当转入资产处置当期的损益，不再予以延。对相关资产划分为持有待售类别的先将尚未分配的递延收益余额冲减相关产的账面价值，再按照《企业会计准则42号——持有待售的非流动资产、处置组终止经营》的要求进行会计处理。
...
1. 净额法
   ...
   **将补助冲减相关资产账面价值。**
   ...
   企业在收到非货币性资产时，应当借记有关资产科目，贷记“递延收益”科目，在相关资产使用寿命内按合理、系统的方法分期计入损益，借记“递延收益”科目，贷记“其他收益”等科目。对以名义金额（1元）计量的政府补助，在取得时计入当期损益。
...
综合性项目政府补助同时包含与资产相关的政府补助和与收益相关的政府补助，企业需要将其进行分解并分别进行会计处理；难以区分的，企业应当将其整体归类为与**收益相关**的政府补助进行处理。
...
认定涉及少量货币性资产的交换为非货币性资产交换，通常以补价占整个资产交换金额的**比例低于25%**作为参考。若补价÷整个资产交换金额<25%，则属于非货币性资产交换；若补价÷整个资产交换金额≥25%的，视为货币性资产交换，适用其他相关准则。

- 【提示】

  （1）补价是公允价值之间的差额，不含增值税之间的差额；

  （2）整个资产交换金额即为在整个非货币性资产交换中**最大的公允价值**。
...
2. 换出资产为固定资产、无形资产的，换出资产公允价值与其账面价值的差额，计入**资产处置损益**。
3. 换出资产为长期股权投资、以公允价值计量且其变动计入其他综合收益的金融资产(债权投资)的，换出资产公允价值与其账面价值的差额，计入投资收益，并将长期股权投资和以公允价值计量且其变动计入其他综合收益的金融资产持有期间形成的“**其他综合收益**”转入投资收益。

##### （三）相关税费的处理

1. 与换出资产有关的相关税费与出售资产相关税费的会计处理相同，如换出固定资产支付的清理费用影响资产处置损益，换出应税消费品应交的消费税计入税金及附加等
...
首先应按换入单项资产的原则确定换入资产成本总额，然后按比例分摊到各个单项资产。
...
非货币性资产交换不具有商业实质，或者虽具有商业实质但换入资产的公允价值不能可靠计量的，应当按照换入各项资产的原账面价值占换出资产原账面价值总额的比例，对换入资产的成本总额进行分配，确定各项换入资产的成本。
...
债务重组，是指在**债务人发生财务困难**的情况下，**债权人**按照其与债务人达成的协议或者法院的裁定**作出让步的事项**。
...
值得注意的是：重组债务和重组债权的确认、终止确认和计量，适用《企业会计准则第22号——金融工具确认和计量》和《企业会计准则第23号——金融资产转移》。
...
1. 债务人的会计处理
   ...
   - 非现金资产为存货的，应当视同销售处理，根据《企业会计准则第14号—收入》相关规定，按存货的公允价值确认销售商品收入，同时按账面价值结转相应的成本。
   - 非现金资产为固定资产、无形资产的，其公允价值和账面价值的差额，计入**资产处置损益。**
   - 非现金资产为长期股权投资等投资性资产的，其公允价值与账面价值的差额，计入投资收益。
...
即期汇率，通常是指中国人民银行公布的当日人民币外汇牌价的中间价。但是，企业发生的外币兑换业务或涉及外币兑换的交易事项，应当按照交易实际采用的汇率（即银行买入价或卖出价）折算。
...
2. 非货币性项目
   ...
   **【提示】**预付款项、预收款项和合同负债属于非货币性项目。
   ...
   - 以历史成本计量的外币非货币性项目，仍采用交易发生日的即期汇率折算，不改变其原记账本位币金额（即不产生汇兑差额）。
   - 对于以成本与可变现净值孰低计量的存货，如果其可变现净值以外币确定，则在确定存货的期末价值时，应先将可变现净值折算为记账本位币金额，再与以记账本位币反映的存货成本进行比较。
   - 以公允价值计量的外币非货币性项目，如交易性金融资产（股票、基金等），采用公允价值确定日的即期汇率折算，折算后的记账本位币金额与原记账本位币金额的差额，作为公允价值变动处理，计入当期损益（公允价值变动损益）。
   - 以公允价值计量且其变动计入其他综合收益的外币货币性金融资产（债务工具）形成的汇兑差额，应当计入**当期损益**，且采用实际利率法计算的该金融资产的外币利息产生的汇兑差额，应当计入当期损益。外币非货币性金融资产（权益工具）形成的汇兑差额，与其公允价值变动一并计入**其他综合收益**，但该金融资产的外币现金股利产生的汇兑差额，应当计入当期损益。
...
对外币报表的折算，常见的方法一般有四种：流动和非流动项目法、货币性与非货币性项目法、时态法和现时汇率法，我国外币折算准则基本采用**现时汇率法**。
...
4. 所有者权益变动表中，当期计提的盈余公积采用当期的平均汇率折算。期初盈余公积为以前年度计提盈余公积按当年平均汇率折算累计而来。期初未分配利润记账本位币金额为以前年度未分配利润记账本位币金额的累计（计算而来而非折算）
...
2. 实质上构成对境外经营净投资的外币货币性项目产生的汇兑差额的处理。
   ...
   - 实质上构成对子公司净投资的外币货币性项目以母公司或子公司的记账本位币反映，则在抵销长期应收应付项目的同时，将其产生的汇兑差额转入“**其他综合收益**”项目。即借记或贷记“财务费用——汇兑差额”科目，贷记或借记“**其他综合收益**”。

- 实质上构成对子公司净投资的外币货币性项目以母、子公司的记账本位币以外的货币反映，则应将母、子公司此项外币货币项目产生的汇兑差额相互抵销，差额转入“**其他综合收益**”。

简单来说，在编制合并财务报表时，如有实质上构成对境外经营净投资的外币货币性项目，因汇率变动而产生的汇兑差额，也应列入所有者权益“**其他综合收益**”；处置境外经营时，计入处置当期损益。

#### **二、境外经营的处置**

企业在处置境外经营时，应当将资产负债表中所有者权益项目下列示的、与该境外经营相关的外币报表折算差额，自所有者权益项目转入处置当期损益；部分处置境外经营的，应当按处置的比例计算处置部分的外币报表折算差额，转入处置当期损益。**处置的境外经营为子公司的，将已列入其他综合收益的外币财务报表折算差额中归属于少数股东的部分，视全部处置或部分处置分别予以终止确认或转入少数股权权益。**
...
1. 分类原则
   ...
   企业主要**通过出售而非持续使用**一项非流动资产或处置组收回其账面价值的，应当将其划分为持有待售类别。
   ...
   **（1）可立即出售**
   ...
   即企业已经就一项出售计划作出决议且获得确定的购买承诺，预计出售将在**一年内**完成。
...
2. 延长一年期限的例外条款
   ...
   因发生罕见情况，导致持有待售的非流动资产或处置组未能在一年内完成出售，企业在最初一年内已经针对这些新情况采取必要措施且**重新满足**了持有待售类别的划分条件。
...
2. 持有待售的长期股权投资
   ...
   企业因出售对子公司的投资等原因导致其丧失对子公司控制权的，无论出售后企业是否保留部分权益性投资，应当在**拟出售**对子公司投资满足持有待售类别划分条件时，在母公司个别财务报表中将对子公司投资整体划分为持有待售类别，在合并财务报表中将子公司所有资产和负债划分为持有待售类别。
...
企业初始计量持有待售的非流动资产或处置组时，其账面价值高于公允价值减去出售费用后的净额的，应当将账面价值减记至公允价值减去出售费用后的净额，减记的金额确认为**资产减值损失**，计入当期损益，同时计提持有待售资产减值准备。
...
非流动资产或处置组因不再满足持有待售类别的划分条件而不再继续划分为持有待售类别或非流动资产从持有待售的处置组中移除时，应当按照以下两者孰低计量：（1）划分为持有待售类别前的账面价值，按照假定不划分为持有待售类别情况下本应确认的折旧、摊销或减值等进行调整后的金额；（2）可收回金额。
...
1. 该组成部分代表一项独立的主要业务或一个单独的**主要经营地区**；
2. 该组成部分是拟对一项独立的主要业务或一个单独的主要经营地区进行处置的一项**相关联计划的一部分；**
3. 该组成部分是**专为转售**而取得的**子公司**。
...
**从财务报表可比性出发，对于当期列报的终止经营，企业应当在当期财务报中将原来作为持续经营损益列报的信息重新作为可比会计期间的终止经营损益列报。**这意味着对于可比会计期间的利润表，作为终止经营列报的不仅包括在可比会计期间即符合终止经营定义的处置组，还包括在当期首次符合终止经营定义的处置组。由于后者的存在，处置组在可比会计期间销售商品、提供劳务的收入和相关成本、费用，以及相关资产按照《企业会计准则第8号——资产减值》的规定确认的资产减值损失等也应当作为终止经营损益列报。企业还应当在附注中披露有关终止经营的相关信息。
...
2. 不再继续划分为持有待售类别的列报
   ...
   对于非流动资产或处置组，如果其不再继续划分为持有待售类别或非流动资产从持有待售的处置组中移除，在资产负债表中，企业应当将原来分类为持有待售类别的非流动资产或处置组重新作为固定资产、无形资产等列报，并调整其账面价值。在当期利润表中，企业应当将账面价值调整金额作为持续经营损益列报。
...
【提示】公司库存股不属于发行在外的普通股。**派发股票股利不考虑时间权数。**
...
$$
稀释每股收益=\frac{净利润+可转债税后实际利息}{普通股加权平均数+可转债转股数\times\frac{存续月数}{12}}
$$
...
增加的普通股数计算公式一：
$$
增加的普通股股数=行权总股数-\frac{行权价格\times行权总股数}{平均价格}
$$
增加的普通股数计算公式二：
$$
增加的普通股数=行权总股数\times\frac{(平均股价-行权价)}{平均股权}\times\frac{存续月份数}{12}
$$
稀释每股收益：
$$
稀释每股收益=\frac{净利润}{普通股加权平均数+增加的普通股加权平均数}
$$
...
存在限制性股票基本每股收益计算公式：
$$
基本每股收益=\frac{净利润-可解锁部分的股利}{普通股加权平均股数}
$$
...
计算行权价
$$
行权价=授予价格+授予日股权公允价值\times\frac{尚未取得的服务年数}{需提供服务总年数}
$$
计算增加的普通股数
$$
增加的普通股数=限制性股票总股数\times\frac{(平均股价-行权价)}{平均股权}\times\frac{存续月份数}{12}
$$
计算稀释每股收益
$$
稀释每股收益=\frac{净利润}{普通股加权平均数+增加的普通股加权平均数}
...
计算增加的普通股数
$$
增加的普通股数=回购总股数\times\frac{(回购价-平均股价)}{平均股权}\times\frac{存续月份数}{12}
$$
计算稀释每股收益
$$
稀释每股收益=\frac{净利润}{普通股加权平均数+增加的普通股加权平均数}
...
$$
本年度基本每股收益=\frac{本期净利润}{配股前股数\times调整系数\times时间权数+配股后的股数\times时间权数}
$$
...
公允价值，是指**市场参与者**在**计量日**发生的**有序交易**中，出售一项资产所能收到或者转移一项负债所需支付的价格。

存货准则中规范的可变现净值、资产减值准则中规范的预计未来现金流量现值等计量属性，与公允价值类似但并不遵循公允价值计量的有关规定，股份支付和租赁业务相关的计量也不遵循公允价值计量的有关规定。

#### **二、公允价值的基本要求**

为了更好地理解公允价值定义，应当从四个方面掌握公允价值计量的基本要求：一是以公允价值计量的相关资产或负债；二是应用于相关资产或负债公允价值计量的有序交易；三是有序交易发生的主要市场或最有利市场；四是主要市场或最有利市场中的市场参与者。
...
1. 相关资产或负债的特征
   ...
   企业以公允价值计量相关资产或负债，应当考虑该资产或负债所具有的特征，例如，**资产的状况及所在位置、出售或使用资产的限制**等。如果市场参与者在计量相关资产或负债公允价值时会考虑这些资产或负债的特征，企业在计量该资产或负债公允价值时，也应当考虑这些特征因素。

   企业为合理确定相关资产的公允价值，应当区分该限制是针对资产持有者的，还是针对该资产本身。如果该限制是针对相关资产本身的，那么此类限制是该资产具有的一项特征，任何持有该资产的企业都会受到影响，市场参与者在计量日对该资产进行定价时会考虑这一特征。因此，企业以公允价值计量该资产时，也应当考虑该限制特征。
...
2. 有序交易价格的应用
   ...
   企业在判定相关资产或负债的交易是有序交易的，以交易价格为基础确定该资产或负债的公允价值，在公允价值计量的过程中赋予有序交易价格的权重时，应当考虑交易量、交易的可比性、交易日与计量日的临近程度等因素。
...
1. 市场参与者的特征
   ...
   市场参与者应当具备下列特征：（1）市场参与者应当相互独立，不存在关联方关系；（2）市场参与者应当熟悉情况，根据可获得的信息，包括通过正常的尽职调查获取的信息，对相关资产或负债以及交易具备合理认知；（3）市场参与者应当有能力并自愿进行相关资产或负债的交易，而非被迫或以其他强制方式进行的交易。
...
市场法是利用**相同或类似**的资产、负债或资产和负债组合的**价格**以及其他相关市场交易信息进行估值的技术。企业应用市场法估计相关资产或负债公允价值的，可利用相同或类似的资产、负债或资产和负债的组合（例如，一项业务）的价格和其他相关市场交易信息进行估值。
...
1. 现金流量折现法。现金流量折现法是企业在收益法中最常用到的估值方法，包括传统法（即折现率调整法）和期望现金流量法。
   ...
   （1）使用合同现金流量：应当采用能够反映预期违约风险的折现率

   （2）使用概率加权现金流量：应当采用无风险利率

   （3）使用包含通货膨胀影响的现金流量：应当采用名义折现率

   （4）使用排除通货膨胀影响的现金流量：应当采用实际利率

   （5）使用税后现金流量的，应当采用税后折现率

   （6）使用税前现金流量的，应当采用税前折现率

   （7）使用人民币现金流量的，应当使用与人民币相关的利率等。
   ...
   第二种方法是，企业在无风险利率之上增加风险溢价，得到期望回报率，并使用该期望回报率对以概率为权重计算的现金流量进行折现，从而估计出相关资产或负债的公允价值。

2. 期权定价模型。

   企业可以使用布莱克斯科尔斯模型、二叉树模型、蒙特卡洛模拟法等期权定价模型估计期权的公允价值。其中，布莱克—斯科尔斯期权定价模型可以用于认股权证和具有转换特征的金融工具的简单估值。布莱克—斯科尔斯期权定价模型中的输入值包括：即期价格、行权价格、合同期限、预计或内含波动率、无风险利率、期望股息率等。

##### （三）成本法

成本法，是反映当前要求**重置相关资产服务能力**所需金额的估值技术，通常是指现行重置成本法。在成本法下，企业应当根据折旧贬值情况，对市场参与者获得或构建具有**相同服务能力**的替代资产的成本进行调整。折旧贬值包括实体性损耗、功能性贬值以及经济性贬值。企业主要使用现行重置成本法估计与其他资产或其他资产和负债一起使用的有形资产的公允价值。
...
第一层次输入值是企业在计量日能够取得的相同资产或负债在活跃市场上**未经调整**的报价。
...
【例题】2×18年12月31日，甲公司在确定该期权公允价值过程中，用到了一种以上的输入值，例如波动率、期望股息率、无风险利率等。其中，无风险利率属于第二层次，波动率和期望股息率属于第三层次。由于波动率和期望股息率对于公允价值计量整体而言是重要的，则该期权的公允价值计量结果应当划入第三层次。

##### （五）第三方报价机构的估值

企业使用第三方报价机构（例如经纪人、做市商等）提供的出价或要价计量相关资产或负债公允价值的，应当确保该第三方报价机构提供的出价或要价遵循公允价值计量的要求。
...
企业以公允价值计量非金融资产，应当考虑**市场参与者**通过直接将该资产用于最佳用途产生经济利益的能力，或者通过将该资产出售给能够用于最佳用途的，其他市场参与者产生经济利益的能力。

最佳用途，是指市场参与者实现一项非金融资产或其所属的一组资产和负债的价值最大化时该非金融资产的用途。最佳用途是评估行业在非金融资产（例如，房地产等）评估中所使用的估值概念，也称为最高最佳使用。企业判定非金融资产的最佳用途，应当考虑该用途是否为**法律上允许、实物上可能**以及**财务上可行**的使用方式。

通常情况下，企业对非金融资产的**当前用途可视为最佳用途**，除非市场因素或者其他因素表明市场参与者按照其他用途使用该非金融资产可以实现价值最大化。
...
1. 会计要素：预算收入、预算支出、预算结余
2. 恒等式：预算收入-预算支出=预算结余
3. 基础：收付实现
4. 报告：决算报告

#### 二、财务会计

1. 会计要素：资产、负债、净资产、收入、费用
2. 恒等式：
   - 资产-负债=净资产
   - 收入-费用=本期盈余
3. 基础：权责发生制
...
4. 权益法调整
   ...
   相当于企业准则中的“资本公积-其它资本公积”

5. 以前年度盈余调整

6. 累计盈余：年末将本年盈余分配转入该科目
...
比较资产负债表上资产负债的账面价值与计税基础，两者之差分为：

- 应纳税暂时性差异
- 可抵扣暂时性差异

并以此确认递延所得税负债与递延所得税资产，从而确定所得税费用。
...
| 无形资产                 | 实际成本-会计累计摊销-减值准备，使用寿命不确定则无摊销 | 自行研发：账面价值*1.75。其它方式：实际成本-税法累计摊销 |
| 以公允价值计量的金融资产 | 当前公允                                                   | 取得时成本                                                   |
| 以摊余成本计量的金融资产 | 摊余成本                                                   | 摊余成本                                                     |
| 投资性房地产             | 当前公允                                                   | 实际成本-税法累计折旧                                        |

（二）负债

|                                | 账面价值     | 计税基础                                                |
| ------------------------------ | ------------ | ------------------------------------------------------- |
| 销售商品等计提的预计负债       | 当前计提金额 | 0                                                       |
| 预收账款                       | 当前金额     | 记计入应纳税所得额：未计入应纳税所得额：当前金额 |
| 现金结算股份支付的职工薪酬     | 当前金额     | 0                                                       |
| 需要缴税的政府补助形成递延收益 | 当前金额     | 0                                                       |
...
1. 职工教育经费、广告宣传费：当年超标后可结转抵扣，产生可抵扣暂时性差异，确认递延所得税资产。
2. 未弥补亏损：结转五年抵扣，产生可抵扣暂时性差异，确认递延所得税资产。
...
1. 递延所得税负债：除明确规定不确认递延所得税的情况外，对于所有的应纳税暂时性差异均应确认相关的递延所得税负债。

2. 递延所得税资产：以未来可能取得的应纳税所得额为限确认。
...
| 资产的减值准备                           | 确认递延所得税资产       |
| 广宣费结转下期抵扣                       | 确认递延所得税资产       |
| 未弥补亏损结转下期继续抵扣               | 确认递延所得税资产       |
| 教育经费结转下期抵扣                     | 确认递延所得税资产       |
| FVOCI                                    | 确认递延所得税资产或负债 |
| 自行研发无形资产加计扣除                 | X                        |
| 权益法长投账面价值与计税基础之差（长持） | X                        |
| 残疾人工资加计扣除                       | X                        |
| 罚款、滞纳金                             | X                        |
| 企业合并购买日形成的商誉                 | X                        |
| 国债利息                                 | X                        |
...
研发无形资产加计扣除，不确认递延所得税资产，理由是：该项交易不是企业合并，交易发生时既不影响会计利润，也不影响应纳税所得额，若确认递延所得税资产，违背历史成本计量属性。

#### 三、特殊事项

1. 直接计入所有者权益的递延所得税

   - 会计政策变更追溯调整法，或前期差错更正采用追溯重述法调整期初留存收益。
   - FCOCI的公允价值变动计入所有者权益
   - 同时包含负债及权益成本的金融工具在初始确认时计入所有者权益

2. 企业合并相关的递延所得税

   购买日存在可抵扣暂时性差异，但当时由于估计后续没有足够的应纳税所得额因而没有确认递延所得税资产的情况，购买日后12个月内发现基于购买日的情况，实际上有足够应纳税所得额可抵扣的，可以确认递延所得税资产并冲减商誉。
...
3. 股份支付相关的当期及递延所得税
   ...
   - 税法规定不准予税前扣除，则没有暂时性差异，不确认递延所得税。
   - 税法规定实际支付时可税前扣除，则正常确认递延所得税。

4. 税率变化

   按新税率算出递延所得税资产或负债的余额，扣减以前期间已确认的递延资产或负债，得到本期发生额。

### 第五节：所得税费用

#### 一、应交税费-应交所得税

应交税费-应交所税税=（税前会计利润+纳税调增额-纳税调减额）X税率

#### 二、递延所得税费用

1. 概念：

   - 递延所税税资产和递延所得税负债对所得税费用的影响
   - 注意：不涉及对其他综合收益的影响

2. 公式：

   递延所得税费用=当期递延所得税负债发生额影响损益部分-当期递延所得税资产发生额影响损益部分

#### 三、所得税费用

1. 所得税费用=应交所得税+递延所得税费用
2. 简单算法
   - 不存在税法的强制性规定也不存在税率变动时：会计利润X税率
   - 存在强制性规定时：考虑强制性规定以后的利润总额X税率
   - 强制性规定包括两部分：一强制性改变会计准则规定（如加计扣除），二是暂时性差异

#### 四、计算步骤

1. 调整会计利润到应纳税所得额

2. 计算每一事项账面价值和计税基础，得到期末可抵扣暂时性差异和应纳税暂时性差异

3. 计算递延所得税资产（负债）本期发生额=期末余额-期初余额

4. 计算所得税费用：

   所得税费用=应效所得税+递延所得税负债发生额对应损益部分-递延所得税资产发生额对应损益部分
...
1. 收入确认原则
   ...
   客户取得相关商品控制权时确认收入。取得商品控制权包括以下三要素：
...
2. 收入确认的前提条件
   ...
   - 对价很可能收回
     ...
     如果不能同时满足上述五个条件，只有在不再负有向客户转让商品的剩余义务，且已向客户收取的对价无需退回时，才能将已收孡的对价确认为收入；否则，已收取的对价作为负债处理。

     后续评估：合同开始日就满足五个条件的，后续无需重新评估（有重大变化迹象除外）。否则，应在后续期间持续评估。

3. 合同合并

   与同一客户或其关联方，同时或相近时间内先后订立超过一份合同，满足下列之一时，应当合并为一分合同：
...
4. 合同变更
   ...
   - 情况一：合同变更部分作为单独合同
     ...
     变更增加了可明确区分的商品及合同价款，且新增合同价款反映了新增商品的单独售价。

   - 情况二：合同变更作为原合同终止及新合同订立

     不属于情况一，且合同变更日已转商品和未转商品可明确区分，应当视为原合同终止；同时，原合同未履约部分与合同变更部分合并为新合同，新合同的交易价格=原合同未确认收入部分+合同变更承诺的对价。

   - 情况三：合同变更部分作为原合同组成部分

     不属于情况一，且合同变更日已转商品和未转商品**不**可明确区分，应将该合同变更部分作为原合同组成部分，在合同变更日重新计算履约进度，并调整当期收入和相应成本等（属于会计估计变更）

##### （二）识别合同中的单项履约义务

应当将下列向客户转让的商品的承诺作为单项履约义务：
...
- 整合及其组合产出转让给客户
- 对合同中承诺的其它商品予以重大修改或定制
- 与合同中承诺的其它商品且有高度关联性

##### （三）确定交易价格

交易价格：因向客户转让商品而预期有权收取的对价金额。代收款项（如增值税）及预期将退还的款项，作为负债处理，不计入交易价格。

1. 可变对价

   - 可变对价=期望值或阳可能发生金额
   - 限制：不超过相关不确定性消除时，累计已确认的收入极可能不会发生重大转回的金额。
   - 每个资表日重新评估

2. 合同中存在的重大融资成分

   - 交易价格=现销价格（即现值）
   - 折现率：名义金额折现为现销价格的折现率，一经确定不得变更。
   - 差额在合同期内用实际利率法摊销，计入财务费用。
   - 取得控制权与客户付款间隔不超一年的，不需要折现。

3. 非现金对价

   - 交易价格=合同开始日的公允价值
   - 公允价值不能确定的，参照其承诺向客户转让商品的单独售价间接确定。
...
4. 应付客户对价
   ...
   - 不构成向客户购买商品
     ...
     按确认收入时间与支付对价时点孰晚冲减收入。

   - 构成向客户购买商品和

     正常采购处理。

     购买价格超过公允的，超过部分作为应付对价冲减交易价格。

     如果公允价不能合理估计，则全部应付对价冲减交易价格。
...
1. **交易价格分摊**
   ...
   市场调整法：根据某商品或类似商品的市场售价，考虑本企业的成本和毛利进行适当调整后确定。

   成本加成法：预计成本加上合理毛利。

   余值法：合同交易价格减去合同中其他商品可观察的单独售价。适用情况为商品近期售价波动幅度巨大，或因没有定价且没有单独销售而使售价无法可靠确定时。

   合同资产与应收账款对比：

   |          | 合同资产                           | 应收账款           |
   | -------- | ---------------------------------- | ------------------ |
   | 收款权利 | 除了时间流逝之外，还取决于其它条件 | 无条件收取合同对价 |
   | 风险     | 信用风险加其他风险（如履约风险）   | 信用风险           |

2. **合同折扣的分摊**

   - 一般情况（整体分摊）：合同折扣在各单项履约义务之间按比例分摊。
   - 特殊情况（部分分摊）：有确切证据表时，折扣仅与部分履约义务相关的，分摊至部分。

3. **可变对价的分摊**

   - 一般情况（整体分摊）：可变对价分摊至合同中的各单项履约义务。对于已履行的履约义务，其分摊的可变对价后续变动额应当调整变动当期收入（限不需要追溯调整）
   - 特殊情况（部分分摊）：满足条件可分摊至部分履约义务。

4. **交易价格后续变动**

   - 一般情况：按在合同开始日所采用的基础，将后续变动金额分摊至合同中的履约义务。（不得因合同开始日之后，单独售价的变动而重新分摊交易价格）
   - 特殊情况：合同变更导致的交易价格后续变动，分三种情况处理：
     - 合同变更情况一：应当判断可变对价后续变动与哪一项合同相关，并按相关规定进行处理。
     - 合同变更情况二：且可变对价后续变动与合同变更前已承诺的可变对价相关的，应当首先将该可变对价后续变动额以原合同开始日确定的单独售价为基础进行分摊，然后再将分摊至合同变更日尚未履行义务的该可变对价额以新合同开始日确定的基础进行二次分摊。
     - 其它情况：应当将该可变对价后续变动额分摊至合同变更日尚未履行（或部分未履行）的履约义务。
...
1. 按时段确认收入
   ...
   - 判断条件（满足之一）：
     ...
     - 客户能够控制企业履约过程中在建的商品。
     - 履约过程中所产出的商品具有不可替代用途，且有权就已完成的履约部分收取款项。

   - 收入确认方法：

     - 原则：应当在该段时间内按照履约进度确认收放，履约进度不能合理确定的除外。

     - 确定方法：

       产出法：根据已转移给客户的商品对于客户的价值确定履约进度。

       投入法：根据企业履约的投入确定履约进度。特殊情况需要适当调整。

     - 计算：

       本期确认的收入=合同交易价格总额X截至本期履约进度-以前期间已确认的收入

       本期确认的成本=合同预计总成本X截至本期履约进度-以前期间已确认的成本

     - 当履约进度不能合理确定时，如已发生的成本预计能够得到补偿的，应当按照已经发生的成本全额确认收入，直到履约进度能够合理确定为止。

2. 按时点确认收入

   - 判断条件：排除法（不属于时段内履行义务的，应当属于时点履行义务）
   - 收入确认方法：在客户取得商品控制权时确认收入。考虑以下迹象：
     - 就该商品有现时收款权利。
     - 已将该商品的法定所有权转移给客户。
     - 已将该商品实物转移给客户（支付手续费方式委托代销除外）
...
2. 非正常消耗
3. 与已履行部分相关的支出。
4. 无法在尚未履行和已履行义务之间区分的相关支出。

##### （二）合同取得成本

为取得合同发生的增量成本，预期能够收回在的，应当人为合同取得成本，确认为一项资产。实务中，该资产摊销期限不超过一年的，可以在发生时计入当期损益。

合同取得成本指企业不取得合同就不会发生的成本（如销售佣金等）

其他支出（如无论是否取得合同都会发生的差旅费），应在发生时计入当期损益（明确由客户承担的除外）

##### （三）资本化部分的摊销

应当采用与该资产相关的商品收入确认相同的基础进行摊销，计入当期损益。
...
1. **发出商品**：
   ...
   贷：主营业务收入（预计不退货部分确认收入）
       预计负债——应付退货款（预计退货部分确认为负债）
          应交税费——应效增值税（销项税额）
   借：主营业务成本（预计不退货部分对应的成本）
          应收退货成本（预计退货部分对应的成本）
   贷：库存商品
...
1. 该条款属于单独服务的：作为单项履约义务处理
2. 该条款与本合同密切相关的：按或有事项处理

#### 三、主要责任人和代理人

1. 主要责任人：转让商品前能够控制该商品的，按总额确认收入
   - 承担转让商品的主要责任
   - 转让前或转让后承担了该商品的风险
   - 有权自主决定售价
2. 代理人：按佣金或手续费确认收入（净额确认）

#### 四、附有客户额外购买选择权

1. 原则：评估该选择权是否向客户提供了一项重大权利。如是，作为单项履约义务，将交易价格分摊至该义务，客户行权或者选择权失效时，确认收入。
2. 客户购买商品时
...
```text
借：合同负债
贷：主营业务收入（金额=税分分摊的成交金额X当期使用/预计总共兑换）
...
1. 收入确认时间
   - 在某一时段确认（同时满足以下）
     - 合同要求或客户能合理预期企业将从事对该项产权有重大影响的活动
     - 该活动对客户将产生有利或不利影响
     - 该活动不会导致向客户转让某项商品
   - 在某一时点确认：不满足在某一时段确认的条件
2. 收入确认金额
   - 多个履约义务：按单独售价分摊交易价格
   - 可变对价（销售提成等）：不按最佳估计数确认，应在以下孰晚确认
     - 客户后续销售或使用行为实际发生
     - 企业履行相关履约义务

#### 六、售后回购

1. 情况一：存在与客户的远期安排而负有回购义务或享有回购权利的，应作为租赁交易或融资交易处理

   - 回购价格小于原售价的

     应当作为租赁交易，原售价减去回购价之差，视为租金，按租赁准则处理。

   - 回购价格不小于原售价的

     应当作为融资交易，收的款项时确认金融负债，收款与回购价的差额在回购期间内确认为利息费用等。

   - 到期不行权的

     在该回购权利到期时终止确认金融负债，同时确认收入。

2. 情况二：负有应客户要求回购商品义务的，分情况：

   1. 如客户有行权重大经济动因的，参照情况一。
   2. 如无重大经济动因，则为附有销售退回条款的销售交易（与一般情况的销售退回一样）。
...
1. 正常履行义务：转为收入
2. 无需退回且客户会放弃部分权利：按比例确认收入
3. 无需退回且客户极低可能要求履约：全额确认收入

#### 八、无需退回的初始费

收取的无需退回的初始费（如入会费）应计入交易价格：

1. 初始费与向客户转让已承诺的商品相关：
   - 该商品构成单项履约义务的，转让该商品时，按照分摊至该商品的交易价格确认收入。
   - 该商品不构成单项履约义务的，应在包含该商品的单项履约义务履行时，按照分摊至该单项履约义务的交易价格确认收入。
2. 初始费与向客户转让已承诺商品不相关：该初始费应作为预收款，在未来转让该商品时确认为收入。

### 第三节：费用

1. 范围：管理费用、销售费用、财务费用。
2. 房产税、车船税、土地使用税、印花税不计入“管理费用”
...
1. 营业利润
   ...
   营业利润=营业收入-营业成本-税金及附加-三项费用-资产减值损失-信用减值损失+其他收益+投资收益（-损失）+净敞口套期收益（-损失）+公允价值变动收益（-损失）+资产处置收益（-损失）
...
4. 综合收益
   ...
   综合收益=净利润+其他综合收益（扣除所得税）

不影响营业利润的只有：营业外收支、所得税。

二、营业外收支

1. 营业外收入：

   非流动资产毁损报废利得、债务重组利得、现金盘盈、捐赠利得。

2. 营业外支出：

   非流动资产毁损报废损失、债务重组损失、公益性捐赠、非常损失、盘亏损失等。
...
企业持有的现金、其它方的权益工具以及符合下列条件之一的资产：

1. 从其它方收取现金或其他金融资产的合同权利。
2. 在潜在有利条件下，与其它方交换金融资产或金融负债的合同权利。
3. 将来须用或可用企业自身权益工具进行结算的非衍生工具合同，且将收到的可变数量的自身权益工具。
4. 将来须用或可用企业自身权益工具进行结算的衍生工具合同，但以固定数量的自身权益工具交换固定金额的现金或其它金融资产的衍生工具合同除外（限固定换固定除外）。
...
1. 以摊余成本计量的金融资产
   ...
   - 现金流特征：本金+利息
   - 核算科目：贷款、应收账款、债权投资
2. 以公允价值计量且其变动计入其它综合收益的金融资产（FVOCI）
   - 业务模式：既以收取合同现金流量为目标，又以出售为目标。
   - 现金流特征：本金+利息
   - 核算科目：其他债权投资
   - 特殊情况：
     - 非交易性权益工具投资（业务模式为长期持有、获取稳定分红、不关心价格波动），直接指定为本类资产。一经指定，不得撤销。
     - 后续处理：
       公允价值的后续变动计入OCI。
       不计提减值准备。
       除获得股利计入当期损益外，其它相关利得和损失均计入OCI，且后续不得转入当期损益。
       终止确认时，之前计入的OCI转入留存收益。
     - 核算科目：其他权益工具投资
3. 以公允价值计量且其变动计入当期损益的金融资产(FVPL)
   - 除划分为上述两类金融资产之外的金融资产
   - 核算科目：交易性金融资产
   - 企业在非同一控制下的企业合并中确认的或有对价构成金融资产的，应当分类为FVPL，不得指定为其他类别。
...
金融工具的重分类日，是指导致企业对金融自进行重分类的业务模式发生变更后的首个报告期间的第一天。也就是变更年度下一年的第一天。
...
3. 在企业具有不同业务模式的各部门之间转移。
   ...
   金融资产条款变更（业务模式没有变更），不导致终止确认时，不允许重分类；导致终止确认的，不属于重分类，视为处置原来的，则入新的金融资产。

（五）具体的重分类计量

1. 以摊余成本计量的金融资产的重分类
   - 重分类为FVOCI：按该金融资产在重分类日公允价值进行计量，原账面公允差计入OCI。
...
2. FVOCI重分类
   - 重分类为摊余成本计量：将之前计入OCI的累计利得和损失转出，调整资产在生分类日的公允价值，即视同该金融资产一直以摊余成本计量。
   ...
   - 重分类为FVPL：应当继续以公允价值计量。同时瘵之前计入OCI的利得或损失转入当期损益。
...
3. FVPL重分类
   ...
   |      | 重分类为摊余成本计量                           | 重分类为FVOCI            |
   | ---- | ---------------------------------------------- | ------------------------ |
   | 原则 | 应当以其在重分类日的公允价值作为新的账面余额。 | 应当继续以公允价值计量。 |
...
#### 三、外币计价的配股权、期权或认股权证，应分类为权益工具

#### 四、特殊金融工具区分

1. 可回售工具：一般符合金融负债的定义。但同时符合一系列条件时，分类为权益工具。
2. 发行方公在清算时才有义务向另一方按比例交付期净资产的金融工具，应分类为权益工具。

#### 五、发行金融工具重分类

1. 权益工具重分类为金融负债

   金融负债按公允价值计算，权益工具账面价值与金融负债公允价值的差额确认为权益。
...
2. 库存股
   ...
   回购自身权益工具（库存股）支付的对价和交易费用，应减少所有者权益，不确认金融资产。

#### 七、复合金融工具

1. 特征：既含有负债成本，又含有权益成本。如可转债。
2. 处理：
   - 发行时：拆分为权益部分和负债部分。发行费用也要拆分。
   - 转换时：终止确认负债部分将将其确认为权益。原来权益部分转至资本公积——股本溢价。

#### 八、永续债

1. 发行方确定永续债会计分类（权益工具或金融负债）时考虑的因素：
...
2. 持有方分类要求：
   ...
   - 属于权益工具投资：划分为FVPL，或FVOCI；
   - 不属于权益工具投资：划分为三类金融资产均可。
...
1. 对不同类别的金融资产，分别以摊余成本、公允价值计量。
2. 如果一项金融工具以前被确认为一项金融资产并以公允价值计量，而现在它的公允价值低于零，则应将其确认为一项负债。
...
2. 后续
   ...
   利息调整=本期投资收益（实际应计利息收入）-现金流入（面值X票面利率）。
...
##### （五）FVOCI（非交易性权益工具）后续计量

1. 初始

   ```text
   借：其他权益工具投资——成本（公允价值加交易费用）
          应收股利（已宣告尚未发放的现金股利）
   贷：银行存款等
   ```
...
注意：处置时损益要转入留存收益，同时记得其他综合收益也要转入留存收益。

#### 三、金融负债的后续计量

##### （一）金融负债后续计量原则

1. FVPL的金融负债：按公允价值计量，利得或损失计入当期损益。
2. 除1及两类特殊情况外：按摊余成本后续计量。
...
减值准备的计提以未来可能的违约事件造成的损失的期望值来计量当前应当确认的损失准备。

企业应对以预期信用损失为基础，对下列项目进行减值会计处理并确认损失准备

1. 以摊余成本计量的金融资产和FVOCI（债权类）
2. 租赁应收款
3. 合同资产
4. 部分贷款承诺和财务担保合同
...
1. 第一阶段：初始确认后信用风险未显著增加
   - 损失准备确认期间：12个月预期信用损失
   - 如何计算利息：按账面总额计算（不扣除信用损失准备）
2. 第二阶段：初始确认后信用风险显著增加，但未发生信用减值
   - 损失准备确认期间：整个存续期内预期信用损失
   - 如何计算利息：按账面总额计算（不扣除信用损失准备）
3. 第三阶段：初始确认后发生信用减值
   - 损失准备确认期间：整个存续期内预期信用损失
   - 如何计算利息：按账面净额计算（扣除信用损失准备）

##### （三）预期信用损失的计量

预期信用损失是以违约概率为权重的，金融工具现金流缺口的现值的加权平均值。

1. 金融资产：信用损失=（应收取的合同现金流量-预期能收取的现金流量）的现值。
2. 折现率：购买或源生已发生信用减值的金融资产，折现率=初始确认时确定的经信用调整的实际利率。
3. 预计预期信用损失期间：指相关金融工具可能发生的现金流缺口所属的期间。
...
2. 该金融资产已转移，且该转移满足本准则关于终止确定的规定。

二、分析所转移金融资产的风险和报酬情况

1. 转移了所有权上几乎所有风险和报酬的应当终止确认。
   - 无条件出信
   - 出售同时约定回购日按公允价值回购
   - 出售同时签订重大价外期权合同
2. 保留了几乎所有的风险和报酬的，应当继续确认该金融资产。
   - 签订了实质性与出售相同的回购协议。
   - 出售时附有将总风险敞口转回的总回报互换。
   - 出售时附有全额补偿条款。
   - 出售时与对方签订重大价内期权。
   - 采用附追索权方式出售。
3. 既没有转移也没有保留金融资产所有权上几乎所有的风险和报酬，应判断是否保留对金融资产的控制，没有保留的，应当终止确认该金融资产。

#### 三、会计处理

将转移前金融资产整体的账面价值，在终止确认部分和继续确认部分之间，按转移日各自的相对公允价值分摊进行账务处理。

### 第六节：套期会计

#### 一、套期

套期，是指企业为管理风险引起的风险敞口，指定金融工具为套期工具，以使套期工具的公允价值或现金流量变动，预期抵销被套项目全部或部分公允价值或现金流量变动的风险管理活动。
...
指企业将套期工具和被套项目产生的得利或损失在相同会计期间计入当期损益（或其他综合收益）以反映风险管理活动影响的方法。

#### 四、套期工具和被套项目

##### （一）套期工具

1. 以公允价值计量且变动计入当期损益的衍生工具（但签出期权除外）。
2. FVPL的非衍生金融资产或金融负债。（但自身FVPL，但信用风险变动引起的公允变动计入OCI的除外）
3. 对于外汇风险，可以指定非衍生金融资产（FVOCI非交易性权益工具除外）或非衍生金融负债的外汇风险成分为套期工具。

对套期工具的指定

1. 应当将符合条件的金融工具整体指定为套期工具，下列除外：
   - 对于期权，可心将期权的内在价值和时间价值分开，只将期权的内在价值变动指定为套期工具。
   - 对于远期合同，可以将远期合同的远期要素和即期要素分开，只将即期要素的价值变动指定套期工具。
   - 对于金融工具，可以将金融工具的外汇基差单独分拆，只将排除外汇基差后的金融工具指定为套期工具。
2. 可以将套期工具的一定比例指定为套期工具。（但不能将套期工具剩余期限内某一时段的公允价值变动部分指定为套期工具。
3. 可以将多项金融工具的组合指定为套期工具。

##### （二）被套项目

指使企业面临公允价值或现金流量变动风险，且被指定为被套期对象的、能够可靠计量 的项目。企业可以将下列单个项目、项目组合或其组成部分指定为被套期项目：

1. 已确认的资产或负债
2. 尚未确认的确定承诺
3. 极可能发生的预期交易。预期交易是指尚未承诺但预期会发生的交易。
4. 境外经营净投资。
5. 项目组成部分。将上述项目整体或项目组合的部分指定为被套期项目。

#### 五、确认和计量

##### （一）公允价值套期

1. 套期工具：产生的利得或损失应当计入当期损益。（如果被套项目是FVOCI，则计入OCI）
2. 被套项目：因被套期风险敞口形成的利得或损失应当计入当期损益，同时调整未以公允价值计量的已确认被套项目的账面价值。（被套项目为FVOCI，其账面价值已按公允计量，不需要调整）

##### （二）现金流量套期

1. 套期工具产生的利得或损失中属于套期有效的部分，作为现金流量套期储备，计入OCI。
2. 套期工具产生的利得或损失中属于套期无效的部分，计入当期损益。
...
1. 披露每一类金融资产和金融负债的公允价值，并与账面价值进行比较。
2. 披露各类金融工具风险相关的定量和定性信息。
...
企业合并是将两个或以上单独的企业合并形成一个报告主体的交易或事项。交易是否构成合并应关注：
...
合并财务报表至少包括：资产负债表、合并利润表、合并所有者权益变动表、合并现金流量表和附注。

#### 二、合并范围的确定

1. 以控制为基础，确定合并范围，具体判断：
   - 享有的是否为可变回报。
   - 判断投资方是否对被投资方拥有权力，并能够运用此权力影响回报金额。
2. 合并范围的豁免——投资性主体：
   - 如果母公司是投资旆主体，只合并为其投资活动提供相关服务的子公司。
   - 对其他子公司应当按公允价值计量且变动计入当期损益。
   - 如果投资性主体的母公司不是投资性主体，则应当将其控制的全部主体纳入合并范围。
   - 如果转换：
     - 原未纳入合并的子公司：在转变日视同公允价值购入，按非同控合并处理。
     - 原纳入合并的会计处理：参照部分处置子公司但不丧失控制权的处理原则。
3. 控制的持续评估：投资方需要评估控制的两个要素中是否发生了变化。

#### 三、合并财务报表的编制原则和程序

1. 编制原则：合并报表除遵循财务报表的一般原则外，还应遵循以下原则：
   - 以个别报表为基础进行编制
   - 一体性原则
   - 重要性原则
2. 前期准备：
...
不形成控股的长期股权投资无须编制合并财务报表。

#### 一、不形成控股合并长投的初始计量

基本原理：实际支付的价款，即“购买价款+交易费用”

1. 为取得长投的各项费用，包括支付的审计费用、资产评估费用、法律咨询费用等，应当于发生时计入初始投资成本：
...
2. 为取得长投发行权益性证券，相关佣金、手续费应当冲减发行证券所确认的溢价收入：

   ```text
   借：长期股权投资
   贷：股本（发行股票数量X面值）
   借：资产公积——股本溢价（权益性证券发行费用）
   贷：银行存款
   ```

3. 为取得长投资发行债券的，所支付的相关费用应当计入所发行债券及其他债务的初始计量金额
...
#### 二、不形成控股合并的长投后续计量（权益法）

##### （一）权益法定义

权益法的核心是长投的账面价值随着被投资单位所有者权益的变动而变动。
...
合营企业和联营企业（即非控股合并形成的长期股权投资）

##### （三）权益法的核算

1. 调初始投资成本
   1. 初始成本大于取得应享有被投资单位可辨认净资产公允价值份额的，本质上属于商誉，不需要对投资成本进行调整。
   2. 初始成本小于取得应享有被投资单位可辨认净资产公允价值份额的，相当于被投资单位的让步，属于“营业外收入”，应当调整长投账面价值
...
2. 调整投资损益（调利润）
   ...
   - 购买时，被投单位公允值与账面价值之差的处理：

     - 以公允价值为基础，对被投单位净利润进行调整。
     - 实现的部分影响被投单位的净利润。

   - 取得长投后，未实现内部交易损益调整：

      内部交易应予以抵销。未实现部分影响净利润。
     ...
     1. 逆流交易的处理：
         ...
         未实现交易在合并报表中的调整（投资单位不合并非控股合并长投的单位，此时为合并其他子公司时的调整）
            借：长期股权投资（在个别报表中已将未实实现的内部交易调减了长投，但实际应当减少的是具体的资产等，所以合并时调整长投和具体的资产）
            贷：存货等
         ```
     ...
     2. 顺流交易的处理：
         ...
         内部顺流交易在合并报表中的抵销（在权益法下，内部交易只能确认其它投资方的，所以不论是否实现，从合表角度上看，均应该抵销该交易产生的收入成本和利润。）
         借：营业收入
         贷：营业成本
            投资收益
         ```
...
3. 取得现金股利或利润的处理
   权益法下取得的现金股利冲减长期股权投资成本，不确认投资收益，所以不影响投资企业利润。因为被投资企业在实现利润时，已经确认了投资收益，所以无需再资确认。而利润分配时，经济利益是流出企业的，所以是减少长投的价值。
...
5. 其他综合收益的处理
   ...
   被投资单位确认的其他综合收益及变动，也会影响投资企业所占份额：
   ...
   - 房地产非投投时，公允价值高出账面差额计入“其他综合收益”（注意：投资性房地产由成本法转公允价值法时，属于会计政策变更，影响留存收益）
   - FVOCI公允价值发生变动时。
...
长投后续计量中无论是权益法还是成本法，如果存在减值迹象，都应该计提减值准备，一经计提，持有期间不得转回。
...
### 第四节：非同一控制下控股合并的长期股权投资信企业合并

#### 一、非同控长投初始计量

处理原则：合并成本为支付的对价的公允价值。为合并支付的交易费用不能计入初始投资成本，只能计入当期损益。

1. 初始投资成本：购买方为企业合并支付的资产、发生或承担的负债、发行权益性证券的公允价值之和为企业合并成本，即长投初始投资成本。
2. 投出物的处理：相当于处置
   - 投出固定资产无形资产：差额计入“资产处置收益”
   - 投出存货：按公允价值确认其他业务收入，账面价值确认其他成本，跌价准备一并结转。
   - 投出金融资产等：差额计入“投资收益”，FVOCI的OCI应一并转。
3. 发生的相关费用处理
   - 各项直接相关费用：计入当期损益（管理费用）
   - 为发行权益性证券支付的相关费用：冲减发行证券所确认的溢价收入（股本溢价）
   ...
   - 发行债券或承担其他债务支付的相关费用：计入所发债券及其他债务的初始计量金额
...
4. 通过多次交易分步实现非同控（个表）
   1. 原投资为长股：购买日长投初始成本=原投资账面价值+新增股份公允价值（原持有的长投因权益法形成的OCI暂不处理，待到处置时再进行处理）
   2. 原投资为金融资产：购买日长投初始成本=原投资公允价值+新增股份公允价值

#### 二、非同控控股合并长投的后续计量

（一）适用范围

对于控股子公司后续计量采用成本法，而非控股的长投，后续计量要采用权益法。

（二）核算方法

1. 成本法下，长投以最初的初始投资成本计量，除非新增投资或减值，长投账面金额不变。
2. 分配现金股利，确认投资收益。
3. 如果将未分配利润或盈余公积转增股本，不用确认投资收益。
...
3. 确定企业合并成本：如前述，反复强调公允价值。

##### （二）非同控取得子公司购买日合并报表编制

1. 初始确认

   一般来说合表确认的长投与个别确认的一致。特殊情况是多次购买形成控股合并时，会存在差异。

2. 计算商誉

   商誉=初始投资成本-被购买方可辨认净资产公允价值（FVINA:Fair Value of Identifiable Net Assets)

3. 调公允：将子公司的账面价值调整到公允价值
...
1. 购买方在购买日将合并中取得的资产、负债按公允价值确认为本期的资产和负债。
2. 作为合并对价的非货币性资产在购买日的公允账面差额，应作为资产处置损益计入当期利润表。
3. 合并成本与所取得FVINA差额，视情况确认为商誉或作为企业合并当期损益计入利润表。
...
借：股本（实收资本）
      ...
      商誉（借方差额）
贷：长期股权投资（母公司）
      少数股东权益（子公司所有者权益X少数股东持股比例）
```

  注意：如果子公司出玑净资产为负数，少数股东按比例承担亏损。即少数股东权益可能出现负债。

（四）第四步：母公司对子公司，子公司相互之间持有对方长投的投资收益抵销
...
### 第五节：同一控制下的的控股合并的长期股权投资及企业合并

#### 一、同控下合并的长投初始计量

1. 处理原则：账面价值原则
2. 初始投资成本：以合并日被合并方账面所有者权益份额作为长投初始投资成本。
3. 初始投资成本付出对价账面差额：计入资本公积——资本溢价。
4. 支付价款中包含已宣告未发放股利的，应作为应收项目处理，不计入合并成本。
...
6. 通过多次交换交易、分步取得股权最终形成同控下控股合并

   在个表中，以持股比例计算的合并日应享有被合并方账面所有者权益份额，作为该项投资的初始投资成本。

   新增投资部分初始投资成本为取得新增部分投资所支付对价的账面价值差额，调整资本公积——资本溢价，资本公积不足冲减的，依次冲减盈余公积和未分配利润。

#### 二、企业合并涉及的或有对价

1. 同控下合并所涉及的或有对价

   不影响长投账面价值，影响的是调整的资本公积——资本溢价。

2. 非同控下合并所涉及的或有对价：

   - 或有对价影响企业合并成本，也影响商誉金额。
   - 如果是购买日存在的情况，在12个月内取得进一步证据发现还需要确认或有对价的，可调整合并成本。
   - 如果超过12个月，或非购买日情况的，不能调整合并成本，可能确认为金融工具（只能为FVPL）。
...
1. 同控下企业合并，不产生新的资产和负债。
2. 被合并方在合并前账面上原确认的商誉作为合并取得资产确认，但合并过程中不产生新的商誉。所以对于产生过商誉的被并购方，初始投资成本为最终控制方合并财务报表上可辨认的净资产份额加上全部商誉。

##### （二）合并日财报编制原则

1. 资产负债表：被合并方的资产、负债以账面价值并入合表。
2. 合并利润表：应包含双方自合并当期期初至合并日实现的净利润，双方当期所发生的交易，应当按照合并财报原则进行抵销。
3. 合并现金流量表：编制原则对合并利润表。
...
1. 取得的资产负债按被合并方原账面价值入账。
2. 付出对价账面与被合并方净资产份额之差，依次冲减资本公积（资本溢价），盈余公积和未分配利润。
3. 交易费用处理与控股合并一样。
4. 吸收合并不存在编制合并报表。

##### （四）通过多次交易分步实现同控下企业合并（合表）

##### （五）同控下取得子公司合并日后合并财报的编制
...
可理解将公允价值核算的金融资产卖掉，然后购入权益法或成本法核算的长投。

1. 长投的初始成本：原股权的公允价值加上新增投资应支付对价的公允价值。
2. 原投资是FVPL的，原投资资转换日公允账面差转入投资收益。
3. 原投资是FVOCI的，原投资于转换日公允账面差，及原累积可转OCI，转入留存收益。

##### （二）权益法转成本法

再重复一下

1. 个别财务报表：长投初始成本为，原权益法下长投账面价值加上新增投资所付的公允价值。
2. 合表：长投初始成本为，原权益法下转换日长投的公允价值加上新增投资所付的公允价值。
   1. 账面和公允的差额计入“投资收益”，但这个投资收益不反应在个表中，而是反映在合表中。
   2. 权益法下长投的OCI在个表中不转入“投资收益”，但在合表中要转入投资收益。
...
1. 处置部分需要确认损益（投资收益）。
2. 剩余部分从账面价值调整到公允价值，差额计入“投资收益”。
3. 总的投资收益=处置部分获得的价款+剩余部分公允价值-初始总成本。
...
1. 处置部分需要确认损益（投资收益）
2. 剩余部分从账面价值调整到公允价值，差额计入“投资收益”。
3. 权益法计量的长投的“其他综合收益”及“资本公积——其他资本公积”全额转“投资收益”。（注意：如果是从权益法到权益法，即仍有重大影响，此时相关部分是按比例结转）

##### （五）成本法转权益法

成本法到权益法，分为个表处理和合表处理，在合表中相当于全部卖掉。

1. 个表处理
   - 处置部分：卖掉的时点需要确认损益（投资收益）
   ...
   - 剩余投资追溯调整权益法账面价值。视同当初买进时即采用权益法核算，调整初始投资成本、投资收益确认等。

2. 投资时点商誉的追溯：正商誉不调整，负商誉调整长投成本，并同时调整留存收益。
3. 投资后的追溯调整：

   ```text
   借：长期股权投资
   贷：留存收益（原投资时至处置当期期初，被投资单位留存收益变动X剩余持股比例）
      投资收益（处置投资当期期初至处置日被投资单位的净损益变动X剩余持股比例）
      其他综合收益（被投单位其他综合收益变动X剩余持股比例）
      资本公积——其他资本公积（其他权益变动X剩余持股比例）
   ```

4. 合表处理

   因为权益法并不需要编制合并报表，所以合并报表相当于把整个长投卖掉。在合表中，应当从成本法调整到权益法入账，所有长期股权投资在合并报表中的账面价值应当是从成本法调整到权益法下的账面价值。

   - 对剩余股权按按丧控日的公允价值重新计量的调整

      ```text
      借：长期股权投资（剩余股权公允价值）
      贷：长期股权投资（剩余股价账面价值，等于个表调整后的账面价值）
         投资收益（本期确认的因为转换实现的收益）

      ```

   - 对个表中的部分处置收益的归属期间进行调整，处置部分增值引起的投资收益增加应转入到权益中去。（还没有想很明白，但从计算上来看，调整到前期的投资收益在数字上等于增值部分X处置比例。

      ```text
      借：投资收益（处置部分引起的增值，包括原子公司利润及其他综合收益等增加引起的）
      贷：留存收益（处置部分利润增加引起的增值）
         其他综合收益等（期货增加引起的增值）
      ```

   - 在子公司终止确认时对其他综合收益进行处理，转入留存收益
...
1. 处置所得价款与长投账面价值差额，应确认为处置损益。
2. 全部处置时，权益法下确认的OCI和资本公积——其他资本公积，应全部转入当期投资收益。
3. 部分处置时，参考长投核算方法转换。
...
##### （一）不考虑存货跌价准备情况下的内部销售业务的抵消

1. 将年初存货中未实现内部销售利润抵消

   ```text
   借：期初未分配利润（年初存货中未实现内部销售利润）
   贷：营业成本
...
2. 然后抵销因本期销售存货结转的存货跌价准备：
   ...
   ```text
   借：营业成本
   贷：存货——存货跌价准备（个别报表的反分录，抵销存货跌价准备之后的销售）

   ```

3. 最后抵消存货跌价准备期末数与上述余额的差额，但存货跌价准备的抵消以存货中未实现的内部销售利润为限
...
1. 调整期初数
   ...
   合表中期初递延所得税资产调整金额即为上期期末合表中递延所得税的调整金额。

2. 调整期初期末差额

   本期期末递延所得税资产的调整金额=合表中递延所得税资产的期末余额-购货方个表中已确认的递延所得税资产
...
1. 需要抵销的内部债权债务项目包括：应收账款应付账款，预付账款预收账款，应收票据应付票据，长期债券投资与应付债券，应收股利与应付股利，其他应收款与其他应付款。
2. 抵销分录
...
##### （三）内部应收款项相关所得税会计的合并抵销处理

1. 先抵消其初坏账准备对递延所得税的影响

   ```text
   借：期初未分配利润（期初坏账准备余额X所得税率）
   贷：递延所得税资产
...
2. 确认递延所得税资产期初期末差额：
   ...
   ```text
   递延所得税期末余额=期末坏账准备余额X所得税率
   如坏账准备期末余额大于期初：
   借：所得税费用（坏账准备增加额X所得税率）
   贷：递延所得税资产

   如坏账准备期末余额小于期初余额
   借：递延所得税资产
   贷：所得税费用（坏账准备备减少额X所得税率）
   ```

#### 三、内部固定资产交易的合并处理

##### （一）未发生变卖或报废的内部交易固定资产抵销

1. 将期初固定资产原价中未实现内部销售利润抵消

   ```text
   借：期初未分配利润
   货：固定资产——原价（期初固定资产原价中未实现内部销售利润）
...
##### （二）发生变卖或报废情况下的内部固定资产交易抵销

1. 将期初固定固定资产原价中未实现内部销售利润抵消
...
（二）发生变卖或报废的内部无形资产交易的抵消

1. 将其实无形资产原价中未实现内部销售利润抵消
...
1. 在已经达到控股的程度，母公司继续购买少数股东持有的股权。
2. 个表处理：按照付出的对价公允价值确认正常长投。
3. 合表处理：因买价与新增持股比例自合并日开始按持续计算INA计算之间的差额，应当调整母公司个别报表中的资本公积，不足则冲减留存收益，不影响商誉。
...
##### （二）因追加投资对非同控下的被投方实施控制

1. 如果属于一揽子交易，应将各项交易作为一项取得子公司控制权的交易进行会计处理。

2. 如果不属一揽子交易，在合表中，对于购买日之前持有的股权，应当按该股权在购买日的公允价值重新计算，公允与账面价值差计入当期投资收益。

   注意：权益法到成本法

   - 个表：长投成本为原股份的**账面价值**加上新增投资的公允价值。
   - 合表：合并成本为原股份的**公允价值**加上新增投资的公允价值。合表需要确认两笔投资收益：公允账面差确认投资收益，其他综合收益转入投资收益。

##### （三）本期增加子公司时如何编制合并财务报表

1. 对于本期投资或追加投资取得的子公司，不需要调整合并资产负债表的期初数。
2. 为了提高会计信息可比性，应到在附注披露本期取得的子公司对合并财务报表的影响。
...
2. 个表：按照收到的价款跟处置的长投成本作比较，确认投资收益。

3. 合表：不确认损益，不影响商誉。差额调整资本公积，不足冲减留存收益。

   合表中确认的资本公积金额=售价-出售日应享有子公司按购买日公允价值持续计算的金额对应处置比例份额。

##### （二）母公司因处置对子公司长投而丧控

1. 一次交易处置子公司
   - 终止确认长投、商誉等账面价值，并终止确认少数股东权益（包括属于少数股东的其他综合收益）的账面价值。
   - 按丧控日的公允价值重新计量剩余股权，作为长投或金融工具核算。
   - 处置对价与剩余股权公允之和，减去按原持股比例计算应享有原股权自购买日开始持续计算的净资产账面价值份额与商誉之和，形成的差额计入丧失控制日当期的投资收益。
   - 与原子公司股权相关的其他综合收益，其他权益变动，可转部分应在丧控日转入当期损益。
2. 多次交易分步处置子公司
   - 不属于一揽子交易：丧失控制前的每一交易，按不丧控部分处置的规定处理。
   - 属于一揽子交易：将各交易作为一项处置原子公司并丧失控制权的交易进行处理。对于丧失控制权之前的每一次交易，价款与持续计算净资产账面份额差，在合表中应当计入**其他综合收益**，在丧失控制权时一并转入丧失控制权当期的损益。

##### （三）本期减少子公司时如何编制合并财务报表

1. 资产负债表不需要再合并
2. 利润表只纳入期初至丧失控制日的收入费用利润。
3. 现金流量表只纳入期初至丧失控制日的现金流量信息。将出售子公司所收到的现金，扣除子公司持有的现金类及相关处置费用后的净额，在有关投资活动类的“处置子公司等收到的现金”项目反映。

#### 三、因子公司少数股东增资导致母公司股权稀释

1. 母公司应按增资前的股权比例计算其在增资前子公司账面净资产中的份额。
2. 该份额与增资后的份额之差，在**合表中**计入资本公积，不足则冲减留存收益。

#### 四、交叉持股的合并处理

1. 母公司持有子公司形成的长投与子公司权益的合抵销处理和通常情况相同。
2. 子公司持有母公司的股权，应当按初始投资成本，将其转为合表中的库存股，作为所有者权益的减项。

#### 五、逆流交易的合并处理

子公司向母公司出售资产，则所发生的未实现内部交易损益，应当按母公司的股权占比在“归属于母公司所有者的净利润”和“少数股东损益”之间分配抵消。
...
#### 二、内部交易固定资产等相关所得税会计合并抵销处理

##### （一）确认本期合表中递延所得税资产期末余额（即列报金额）

1. 递延所得税资产的期末金额=期末合表中固定资产可抵扣团里性差异余额X税率。
2. 合表中固定资产账面价值为内部销售方期末固定资产的账面价值。
3. 合表中固定资产的计税基础是集团内部购货方期末按税法规定确定的账面价值。

##### （二）调整合表中本期递延所得税资产

本期期末递延所得税资产的调整金融=合表中递延所得税资产的期末余额-购货方个表中已确认的递延所得税资产
...
1. 列报基础：持续经营。在非持续经营情况下，应当在附注中声明。
2. 现金流量表采用收付实现制，其他报表采用权责发生制。
3. 关于抵消：一般不得抵销，以下三种情况不属于抵消：
   - 一组类似交易形成的利得或损失，但有重要性的除外。
   - 资产负债表按扣除科备抵项目后的净额列示，不属于抵消。
   - 非日常活动利得损失：如出售处置固定资产等。
...
1. 应收票据和应收账款，归并为“应收票据及应收账款”，应付类似。
2. 应收利息、应收股利，归入其他应收款，应付类似。
3. 固定资产清理：归入固定资产。
4. 工程物资：归入在建工程。
...
1. 根据总账科目填列：
2. 根据有关明细科目的余额分析计算填列：一年内到期的，开发支出等。
3. 根据总账科目和明细账科目的余额分析计算填列：长期借款项目=长期借款总账余额-一年内到期且不能自主展期。
4. 根据有关科目余额减去其备抵科目后的净额填列：长投等。
5. 综合运用上述填列方法填列。
...
1. 从管理费用项目中分拆“研发费用”项目。
2. 增设“信用减值损失”

二、其他事项

1. 营业收入=主营业务收入+其他业务收入，销售退回等反方向冲减。
2. 其他综合收益：期末未在当期损益中确认的各项利得和损失的税后净额。
3. 综合收益总额=净利润+其他综合收益（税后）
4. 影响营业利润的因素中，包括“资产处置收益”，“其他收益”，“净敞口套期收益”，“信用减值损失”。

### 第四节：现金流量表

一、编制基础
以现金和现金等价物为基础编制，按照收付实现制编制。

1. 现金：包括库存现金，和可随时用于支付的存款。主要包括：库存现金、银行存款、其他货币资金。
2. 现金等价物：期限短（三个月内）、流动性强、易于转换为已知金额现金、价值变动风险很小的投资。
3. 现金等价物小于货币性资产。现金等价物范围一经确定不得随意变更。

二、现金流量表组成
（一）经营活动现金流量

1. 销售商品提供劳务收到的现金：在营业收入基础上加减调整。
2. 购买商品、接受劳务支付的现金。
3. 支付给职工以及为职工支付的现金。
...
1. 收回投资收到的现金
2. 取得投资收益收到现金
3. 处置子公司及其他营业单位收到现金净额
4. 取得子公司及其他营业单位支付的现金净额
   - 吸收合并：购买支付的现金减去合并单位持有的现金。
   - 控股合并：为支付对价现金支付（合并后还是两个单位，不需要抵减）
5. 购建固定资产、无形资产和其他长期资产支付的现金：
   - 注意：不包括分期付款的首付款及各期款（在“支付与其他筹资活动有关的现金”项目中反映）
   - 购建活动发生的贷款利息，资本化费用化的均在“分配股利、利润或偿付利息支付的现金”中反映。

（三）筹资活动现金流量

1. 吸收投资收到的现金
2. 偿还债务支付现金：只包括本金。偿还利息在“分配股利、利润或偿付利息支付的现金”列示。
3. 支付其他与筹资活动相关的现金：融资租入固定资产各期支付的租金。分期付款方式购建的固定资产等。
...
1. 日常往来资金提供者、公用事业部门、政府、供应商、客户。
2. 合营公司的合营者之间不是。
3. 同受国家控制而无其他影响的。
4. 同受同一方重大影响（非控制）的公司间不是。

三、披露要求

1. 无论是否发生关联交易，均需要披露子公司、母公司信息。
2. 发生关联交易，披露关联方关系的性质、交易类型及要素。
3. 提供合并报表，合并范围内各企业之间交易不用披露。
...
1. 被购买方是否构成业务
   - 不构成业务
     - 将购买成本按照购买日各项资产负债的相对公允价值分配，不按企业合并准则处理。
     - 差额分解到各项资产负债中，不产生商誉。
     - 交易费用：计入购买资产成本。
     - 暂时性差异不确认递延所得税。
   - 构成业务
...
1. 确认长期股权投资时，根据或有事项准则，判断是否就或有对价确认预计负债或预计资产。
2. 后续或有对价的实际结算金额与合并时估计的金额不一致的，差额调整资本公积，不影响当期损益。

（二）非同一控制下

1. 购买日估计或有对价，并将其计入合并成本。
2. 或有对价的变化，分情况处理：
   - 购买日12个月内出现对购买日已存在情况的新证据，需要调整或有对价的，调整商誉。
   - 其他情况变化
     - 或有对价为权益性质的，不进行会计处理。
     - 或有对价为金融工具的，按FVPL处理，不得划分为其它类别。

### 第二节：反向购买

一、定义
一般而言，发行权益证券的一方为购买方，但：发行后，发行方的经营决策被参与方所控制，那么，发行方为法律上的购买主但其为会计上的被购买方，即：反向购买。

二、企业合并成本
反向购买中，法律上的子公司（购买方）的企业合并成本是指如果以发行权益性证券的方式为获取在合并后报告主体的股权比例，应当向法律上的母公司（被购买）方发行的权益性证券数量与公允价值计算的结果。

三、合并报表
法律上的母公司站在法律上的子公司角度编制合并报表。或者说被购买方站在购买方的角度编制合并报表

1. 合表的资产负债：等于购买方资产负债的账面价值加上被购买方资产负债公允价值，被购买方账面价值与公允价值不一致时，应当调整。
2. 合并报表中的留存收益和其他权益性余额应当反映购买方在合并前的留存收益和其他权益余额。
3. 合并报表股本金额等于购买方合并前发行的股份面值加上本次计算合并成本时模拟发行的股份面值。股本数量等于被购买方原有的，加上本次发行的。
4. 购买方虚拟发行的合并成本与购买日享有的被购买方可辨认净资产公允价值份额（此份额为法律上的被购买方占购买方的比例），多付计商誉，少付计损益。
5. 合并报表的比较信息为购买方的比较信息。

四、每股收益
收益按合并收益计算。
股份数按被购买方发行股份数的加权平均。

五、非上市公司购买上市公司股权实现间接上市

1. 交易发生时，上市公司不构成业务：上市公司编制合表时，不确认商誉或损益。
2. 交易发生时，上市公司构成业务：确认商誉或损益。
